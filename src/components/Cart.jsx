...
import product_checkout2 from "../assets/product_checkout_2.png";
...
function Cart() {
    ...
    const handleClicktest = (link, obj) => {
        ...
        localStorage.setItem('quantity16Items', quantity);
        localStorage.setItem('quantity8Items', quantitySecond);

        localStorage.setItem('price16Items', price * quantity);
        localStorage.setItem('price8Items', priceSecond * quantitySecond);
    ...
    const increasePrice = (x) => {
        const storedItems = JSON.parse(localStorage.getItem('selectedItems')) || [];
        const indexToUpdate  = storedItems.findIndex(item => item.id === x.id);
        const actualPrice= details.filter(y => y.id === x.id);
        if (indexToUpdate  !== -1) {
            storedItems[indexToUpdate].price += actualPrice[0].price
            storedItems[indexToUpdate].unit += 1;
            localStorage.setItem('selectedItems', JSON.stringify(storedItems));
            setCartItems(storedItems);
        }
    };
    const decreasePrice = (x) => {
        const storedItems = JSON.parse(localStorage.getItem('selectedItems')) || [];
        const indexToUpdate  = storedItems.findIndex(item => item.id === x.id);
        const actualPrice= details.filter(y => y.id === x.id);
        if (indexToUpdate  !== -1) {
    ...
    const removeItem = (itemId) => {
        const storedItems = JSON.parse(localStorage.getItem('selectedItems')) || [];
        const indexToRemove = storedItems.findIndex(item => item.id === itemId.id);
        if (indexToRemove !== -1) {
    ...
    return (
        <>
            ...
            <section className="cart_sec inner-banner-section IntelliVista_banner">
                <div className="container">
                    <div className="row">
                        <div
                            className="col-lg-6 col-md-12 col-sm-12 col-12 d-flex align-items-center">
                            <ul className="card_breadcrumb">
                                <li className="active">
                        ...
                        <div
                            className="col-lg-6 col-md-12 col-sm-12 col-12 d-flex align-items-center justify-content-end">
                            <button className="btn btn-primary w-auto" onClick={handleClicktest} type="submit"
                                value="Submit">Add shipping
                                details</button>
                        </div>
                    </div>
                    ...
                    {cartItems && cartItems.length > 0 && (
                        ...
                        <div className="row mt-4 pt-lg-2">
                            ...
                            <div className="col-lg-8 col-md-9 col-sm-12 col-12 d-flex align-items-start">
                                <div className="card_title">
                                    <h6 className="mb-0">{x.subHeading}</h6>
                                    <p
                                        className="d-flex align-items-center text-start gap-4 mt-4"><b>Price
                                            :</b> <b>${x.price}</b></p>
                                    <div
                                        ...
                                        <div
                                           className={`previous_button ${x.unit === 1 ? 'disabledData' : ''}`}
                                           onClick={() => x.unit !== 1 && decreasePrice(x)}
                                            >
                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                    width="24" height="24"
                                                    viewBox="0 0 24 24" fill="none">
                                                    <g opacity={x.unit == 1 ? "0.25" : ""}>
                                                        <path
                                            ...
                                            <div>
                                                <p id="quantity" className="m-0"><b> <b>{x.unit}</b></b></p>
                                            </div>
                                            <div className="next_button" onClick={()=>increasePrice(x)} >
                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                    width="24" height="24"
                                                    viewBox="0 0 24 24" fill="none">
                                                    <path
                                    ...
                                    <p className="mt-4 text-start"><b>Shelf life : {x.shelf_life}</b></p>
                                    <p className="text-start mt-3 mb-0">{x.pecriptionHeading}</p>
                                    <p className="text-start mt-3 mb-0">{x.pecription}</p>
                                    <button className="btn btn-outline-primary w-auto d-flex align-items-center px-3 mt-4"  onClick={() => removeItem(x)}>
                                        <svg xmlns="http://www.w3.org/2000/svg"
                    ...
                    {/* <div className="row mt-4 pt-lg-2">
                        ...
                        <div className="col-lg-8 col-md-9 col-sm-12 col-12 d-flex align-items-start">
                            <div className="card_title">
                                <h6 className="mb-0">Scentaware 8-scent Smell Test and Training kit</h6>
                                <p
                                    className="d-flex align-items-center text-start gap-4 mt-4"><b>Price
                                        :</b> <b>${39.99 * quantitySecond.toFixed(2)}</b></p>
                                <div
                                    className="d-flex align-items-center gap-4 mt-3">
                                    <p className="m-0 text-start">
                                        <b>Qty. :</b>
                                    ...
                                    <div className="d-flex align-items-center gap-3">
                                        <div className="previous_button" onClick={decreaseQuantitySecond}>
                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                width="24" height="24"
                                                viewBox="0 0 24 24" fill="none">
                                                <g  opacity={quantitySecond === 1 ? "0.25" : ""}>
                                                    <path
                                                        d="M14.4001 16.8099L9.6001 12.0099L14.4001 7.20988"
                                                        stroke="#031A40"
                                                        stroke-width="2"
                                        ...
                                        <div>
                                        <p id="quantity" className="m-0"><b> <b>{quantitySecond}</b></b></p>
                                        </div>
                                        <div className="next_button" onClick={increaseQuantitySecond} >
                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                width="24" height="24"
                                                viewBox="0 0 24 24" fill="none">
                                                <path
                                ...
                                <p className="mt-4 text-start"><b>Shelf life : 6
                                    months</b></p>
                                <p className="text-start mt-3 mb-0">Banana, Lemon, Peppermint, Garlic, Cinnamon, Orange, Coffee, Rose.</p>
                                <button className="btn btn-outline-primary w-auto d-flex align-items-center px-3 mt-4">
                                    <svg xmlns="http://www.w3.org/2000/svg"
                                        width="20" height="20" viewBox="0 0 20 20"
                                        fill="none">
                                        <g clip-path="url(#clip0_3250_2380)">
        ...
        // <Wrapper>
        //   <section className="cart-section" id="shop">
        ...
        //                       data-bs-target={`#${dynamicId}`} data-bs-slide="prev"> 
        //                   <span className="carousel-control-prev-icon" aria-hidden="true"></span> 
        //                   <span className="visually-hidden">Previous</span>
        //                   </button> 
        //                   <button className="carousel-control-next" type="button"
        //                       data-bs-target={`#${dynamicId}`} data-bs-slide="next"> 
        //                   <span className="carousel-control-next-icon" aria-hidden="true"></span> 
        //                   <span className="visually-hidden">Next</span>
        //                   </button> 
        //             </div>
        //           </div>
        //           <div className="col-lg-6 col-md-6 col-sm-12 cart-alignment">
        //             <div className="col-lg-12">
        //               <p className="cart-main-header">
        //                 {main_header}Evaluation and Therapy kit
        //               </p>
        //               <p className="cart-des">
        //                 {desc}
        //                 <br />
        //                 <h3 className="cart-des">{list[0]}</h3>
        //                 <h3 className="cart-des"> {list[1]} </h3>
        //                 <h3 className="cart-des">{list[2]} </h3>
        ...
        //                   <span className="qty-hide">Qty :</span>
        ...
        //                   <div className="">
        //                     <button
        //                       className="btn-add-to-cart ripple"
        ...
        //                 Note :
        //                 <span>
        //                 To be used with SensifyAware App for best results.
        //                 </span>
        //               </p>
        //             </div>
        //           </div>
...
//       padding-right:0px;
//     }
//     .cart-content .qty-block span img {
//       padding-left : 10px !important;
//       margin-right: 0px !important;
//       margin-bottom: 0px !important;
//       margin-top: 0px !important;
//     }
//     .cart-content .qty-block span img:nth-child(1) {
